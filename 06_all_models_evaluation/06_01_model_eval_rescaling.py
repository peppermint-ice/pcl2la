...
def plot_distributions(train_set, test_set, fold_num, scaler, before=True):
    ...
    train_rescaled['measured_leaf_area'] = inverse_transform(train_set['measured_leaf_area'], scaler.mean_[-1], scaler.scale_[-1])
    test_rescaled['measured_leaf_area'] = inverse_transform(test_set['measured_leaf_area'], scaler.mean_[-1], scaler.scale_[-1])
...
if __name__ == '__main__':
    ...
    # Apply top limit for measured leaf area at 5000
    y_test_all_limited = [y for y in y_test_all if y <= 5000]
    y_test_all_scaled_limited = [y_test_all_scaled[i] for i in range(len(y_test_all)) if y_test_all[i] <= 5000]
    aggregated_y_test_pred_limited = [y_test_pred_all[i] for i in range(len(y_test_all)) if y_test_all[i] <= 5000]
    ...
    threshold_limited = np.percentile(errors_limited, 90)
    ...
    plt.ylabel('Measured')
    plt.xlabel('Predicted')
    plt.title(f'Aggregated Predictions vs True Values (Limited to 5000)\nR2: {r2_limited:.4f}')
    plt.legend()
    plt.tight_layout()
    plt.show()

    # Print the 10% worst points for aggregated predictions with limitation
    print("10% worst points (measured leaf area values in scaled form) with limitation to 5000:")

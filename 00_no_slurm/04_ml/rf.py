...
if __name__ == '__main__':
    ...
    file_path = os.path.join(folder_paths["ready_for_training_2"], "marching_cubes_13_o3d_simple_noElim.csv")
    ...
    try:
        ...
        for i, (train_index, test_index) in enumerate(kf.split(df)):
            ...
            model_filename = f"{parameter_name}_{parameter_value}_{assessment_name}_{repaired}_{eliminated}_2_model_kf_rf_fold_{i}.pkl"
            model_filepath = os.path.join(model_folder_path, model_filename)
            with open(model_filepath, 'wb') as f:
                pickle.dump(model, f)

            # Save train and test datasets as CSV files
            train_filename = f"{parameter_name}_{parameter_value}_{assessment_name}_{repaired}_{eliminated}_2_train_kf_rf_fold_{i}.csv"
            ...
            test_filename = f"{parameter_name}_{parameter_value}_{assessment_name}_{repaired}_{eliminated}_2_test_kf_rf_fold_{i}.csv"
            ...
            output_file = f"{parameter_name}_{parameter_value}_{assessment_name}_{repaired}_{eliminated}_2_results_kf_rf.csv"
